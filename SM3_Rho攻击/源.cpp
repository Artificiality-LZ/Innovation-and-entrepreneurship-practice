...
#include<map>
using namespace std;
const unsigned int bool_iv_length = 1;
struct SM3_B
{
	unsigned int B[16] = { 0 };
	SM3_B* next = nullptr;
...
struct SM3_V
{
	...
	bool operator < (const SM3_V& pk) const
	{
		for (int i = 0; i < 8; i++)
		{
			if (this->V[i] < pk.V[i])
...
struct SM3_IV_BOOL
{
	unsigned int IV[bool_iv_length] = { 0 };
	bool operator < (const SM3_IV_BOOL& pk) const
	{
		for (int i = 0; i < bool_iv_length; i++)
		{
			if (this->IV[i] < pk.IV[i])
			{
				return true;
			}
			else if (this->IV[i] == pk.IV[i])
			{
				continue;
			}
			else
...
int main()
{
	map<SM3_IV_BOOL, SM3_B> test_map;
	SM3_B te;
	SM3_IV_BOOL test_iv;
	SM3_B target;
	map<SM3_IV_BOOL, SM3_V> IV_map;
	SM3_V IV;
	SM3_V IV_target;
	srand(time(0));
	for (int i = 0; i < 8; i++)
	...
	while (true)
	{
		...
		if (test_map.count(test_iv) == 0)
		{
			test_map.insert({ test_iv,te });
			IV_map.insert({ test_iv,IV });
			for (int i = 0; i < 8; i++)
			{
				te.B[i] = IV.V[i];
			}
		...
		{
			target = test_map[test_iv];
			IV_target = IV_map[test_iv];
			break;
		}
